...
const customer = (plg, event, customerId) => {
  ...
  const refundedPayments = async (plg, event, offset) => {
    const headers = { access_token: `${event.auth.api_key}` };
    ...
    return plg.axios({
      ...
      method: 'GET',
      params: {
        'refundedDateTime[ge]': parsedLastReqAt,
        'status': 'REFUNDED',
        'limit': limit,
        'offset': offset
      }
      ...
      if (data.hasMore) {
        ...
        const morePayments = await refundedPayments(plg, event, newOffset);
  ...
  exports.handle = async (plg, event) => {
    const allPayments = await refundedPayments(plg, event, 0);
    console.log("Number of payments refunded", allPayments.length);
