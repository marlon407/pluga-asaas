...
const receivedPayments = async (plg, event, offset) => {
  const headers = { access_token: `${event.auth.api_key}` };
  ...
  return plg.axios({
    ...
    if (data.hasMore) {
      ...
      const morePayments = await receivedPayments(plg, event, newOffset);
...
exports.handle = async (plg, event) => {
  const allPayments = await receivedPayments(plg, event, 0);
  console.log("Number of payments received today", allPayments.length);
